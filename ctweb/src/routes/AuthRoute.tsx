...
export const AuthRoute = ({ render, redirect, ...props }) => {
  React.useEffect(() => {
    ...
    if (window.location.href.indexOf("?code=") > -1) {
      const authCode = decodeURIComponent(window.location.href)
        .split("?code=")[1]
        .split("&state=")[0];
      ...
      params.append(
        "code_verifier",
        localStorage.getItem(OAUTH.pkceCodeVerifier) || ""
      );
      axios
        ...
        .then(function (response) {
          ...
          window.location.href = decodeURIComponent(window.location.href).split(
            "?code="
          )[0];
          return response;
        })
    ...
    /* Initial redirect to authorization endpoint */
      ...
      (async function redirection() {
        ...
        const redirect =
          ...
          "&scope=" +
          encodeURIComponent(OAUTH.requestedScopes) +
          "&redirect_uri=" +
